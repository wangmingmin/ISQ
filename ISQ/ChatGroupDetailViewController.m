    ...
    for (i = 0; i < row; i++) {
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                UIImageView * headImg=[[UIImageView alloc]initWithFrame:CGRectMake(j * kContactSize, i * kContactSize, kContactSize, kContactSize)];
       ...
       //群组联系人头像名称
                ...
                contactView.image = [UIImage imageNamed:@"defuleImg"];
                ...
                for(NSDictionary *dic in _friendsDataSource[@"friends"]){
                    ...
                    if([dic[@"hxid"] isEqualToString:username]){
                        
                        NSURL *imgUrl=[[NSURL alloc]initWithString:[NSString stringWithFormat:@"http://%@",dic[@"avatar"]]];
                        
                        [headImg setImageWithURL:imgUrl placeholderImage:[UIImage imageNamed:@"placeholder-avatar"]];
                        
                        contactView.remark = dic[@"nick"];
                        contactView.image = headImg.image;
                    }
