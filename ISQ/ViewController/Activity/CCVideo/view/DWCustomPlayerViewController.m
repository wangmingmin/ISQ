...
- (void)loadBackButton
{
    ...
    [self.backButton setTitle:[NSString stringWithFormat:@"< %@",self.videoTitle] forState:UIControlStateNormal];
...
- (void)checkNetworkState
{
    ...
    Reachability *conn2 = [Reachability reachabilityForInternetConnection];
    
    if ([wifi currentReachabilityStatus] == NotReachable) { // 没有使用wifi, 使用手机自带网络进行上网
        
       [NSTimer scheduledTimerWithTimeInterval:1.5 target:
                           self selector:@selector(function) userInfo:nil repeats:NO];
        NSLog(@"使用手机自带网络进行上网");
...
-(void)function{
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"wifi已经断开，是否继续？" delegate:self cancelButtonTitle:@"停止" otherButtonTitles:@"继续", nil];
    [alert show];
