...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate
{
    ...
    if (self) {
        ...
        //初始化默认显示view
        CGFloat sectionWidth = 70;
        for (int i = 0; i <sectionNum; i++) {
            UIButton *sectionBtn = [[UIButton alloc] initWithFrame:CGRectMake(300, 1, sectionWidth, frame.size.height-2)];
            sectionBtn.titleLabel.font = [UIFont systemFontOfSize:13];
            sectionBtn.titleLabel.textColor = [UIColor darkGrayColor];
            ...
            UIImageView *sectionBtnIv = [[UIImageView alloc] initWithFrame:CGRectMake(sectionBtn.left-10, (self.frame.size.height-15)/2, 15, 15)];
...
-(void)showChooseListViewInSection:(NSInteger)section choosedIndex:(NSInteger)index
{
    ...
    //设置弹出tableView的高度
    rect .size.height = [self.dropDownDataSource numberOfRowsInSection:currentExtendSection]*50;
    [UIView animateWithDuration:0.3 animations:^{
