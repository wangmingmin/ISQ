...
-(void)getCallerData:(NSString*)chatter{
    ...
    if(_friendsDataSource[@"friends"]){
        ...
        ISQLog(@"sfdsfsfsafsafsafsafsaf%@",_friendsDataSource);
        
        for (NSDictionary *searchData in _friendsDataSource[@"friends"]) {
  
            if ([searchData[@"hxid"] isEqualToString:chatter]) {
                
                _nameLabel.text = [NSString stringWithFormat:@"%@",[searchData[@"remark"] length]>0 ?searchData[@"remark"] :searchData[@"nick"]];
                
                NSURL *imgUrl=[[NSURL alloc]initWithString:[NSString stringWithFormat:@"http://%@",searchData[@"avatar"]]];
                [_headerImageView setImageWithURL:imgUrl placeholderImage:[UIImage imageNamed:@"placeholder-avatar"]];
...
//    NSDictionary *arry=@{@"friend":chatter};
...
//            
//            _nameLabel.text = Frindsdata[@"data"][@"nick"];
//            NSURL *imgUrl=[[NSURL alloc]initWithString:[NSString stringWithFormat:@"http://%@",Frindsdata[@"data"][@"avatar"]]];
//            [_headerImageView setImageWithURL:imgUrl placeholderImage:[UIImage imageNamed:@"placeholder-avatar"]];
//        }
//            
//   
...
//    }
    ...
    if ([[user_info objectForKey:Shating_switch] isEqualToString:@"YES"]) {
        // 收到消息时，震动
                           ...
                           error:(EMError *)error
{
    ...
    if(error){
        ...
        [self _insertMessageWithStr:@"未接听"];
        
        
        if([error.description rangeOfString:@"online"].location!=NSNotFound ){
            
            
            UIAlertView *errorAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"error", @"Error") message:@"对方并未在线！" delegate:self cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
            errorAlert.tag = kAlertViewTag_Close;
            [errorAlert show];
            
            
            
        }else if([error.description rangeOfString:@"reachable"].location!=NSNotFound ){
            
            
            UIAlertView *errorAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"error", @"Error") message:@"服务器未响应！" delegate:self cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
            errorAlert.tag = kAlertViewTag_Close;
            [errorAlert show];
        }
