...
- (void)viewDidLoad
{
    ...
    //颜色
//    [self.navigationController.navigationBar setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIColor whiteColor],NSForegroundColorAttributeName,[UIFont fontWithName:@"Marion-Italic" size:18.0],NSFontAttributeName,nil]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.searchData.count>0) {
        ...
        NSURL *imgUrl=[[NSURL alloc] initWithString:[NSString stringWithFormat:@"http://%@",self.searchData[@"data"][@"avatar"]]];
        
        cell.imageURL =imgUrl;
        cell.textLabel.text = self.searchData[@"data"][@"nick"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *buddyName = self.searchData[@"data"][@"hxid"];
...
//        [self showMessageAlertView];
        ...
        [CmdDealWith cmdToDealWith:CMD_ACTION_NOTICE_ADD :self.searchData[@"data"][@"hxid"]];
...
-(void)searchFriendsHttp{
    ...
    NSDictionary *arry=@{@"friend":_textField.text};
    ...
    [manager GET:SEARCHFRIENDSURL parameters:arry success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [self hideHud];
        self.searchData=[NSJSONSerialization JSONObjectWithData:responseObject options:NSJapaneseEUCStringEncoding error:nil ];
        
        if ([self.searchData[@"code"] isEqualToString:@"0"]) {
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:@"没有发现此用户！" delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
            [alertView show];
         ...
         [self hideHud];
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:@"请检查网络连接！" delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
        [alertView show];
