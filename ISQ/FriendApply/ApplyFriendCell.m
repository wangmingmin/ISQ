...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _headerImageView.layer.borderColor=[[UIColor lightGrayColor    ]colorWithAlphaComponent:0.3f].CGColor;
        ...
        _iknownButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 60, 0, 50, 30)];
        [_iknownButton setBackgroundColor:[UIColor colorWithRed:10 / 255.0 green:82 / 255.0 blue:104 / 255.0 alpha:1.0]];
        [_iknownButton setTitle:@"知道了" forState:UIControlStateNormal];
        _iknownButton.clipsToBounds = YES;
        [_iknownButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _iknownButton.titleLabel.font = [UIFont systemFontOfSize:14.0];
//        [_iknownButton addTarget:self action:@selector(addFriend) forControlEvents:UIControlEventTouchUpInside];
        [self.contentView addSubview:_iknownButton];
...
-(void)layoutSubviews
{
    ...
    _refuseButton.frame = CGRectMake(self.contentView.frame.size.width - 120, (self.contentView.frame.size.height - 30) / 2, 50, 30);
     _iknownButton.frame = CGRectMake(self.contentView.frame.size.width - 60, (self.contentView.frame.size.height - 30) / 2, 50, 30);
