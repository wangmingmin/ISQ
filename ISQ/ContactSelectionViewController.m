...
@property (strong, nonatomic) NSDictionary *friendsDataSource;
...
-(void)loadMyfriends{
    ...
    EGOCache *theCache=[[EGOCache alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.imageView.layer.borderWidth=0.3f;
    cell.imageView.layer.borderColor=[[UIColor lightGrayColor]colorWithAlphaComponent:0.4f].CGColor;
    
    cell.imageView.layer.cornerRadius=3.0f;
    cell.imageView.layer.masksToBounds=YES;
   
    cell.imageView.image=[UIImage imageNamed:@"defuleImg"];
    cell.textLabel.text = buddy.username;
    
    for(NSDictionary *dic in _friendsDataSource[@"friends"]){
        
        
        
        if([dic[@"hxid"] isEqualToString:buddy.username]){
            
            
            NSURL *imgUrl=[[NSURL alloc]initWithString:[NSString stringWithFormat:@"http://%@",dic[@"avatar"]]];
            
            [cell.imageView setImageWithURL:imgUrl placeholderImage:[UIImage imageNamed:@"placeholder-avatar"]];
            cell.textLabel.text = dic[@"nick"];
...
- (void)reloadFooterView
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        EMRemarkImageView *remarkView = [[EMRemarkImageView alloc] initWithFrame:CGRectMake(i * imageSize+3, 0, imageSize, imageSize)];
        UIImageView *footerHead=[[UIImageView alloc]initWithFrame:CGRectMake(i * imageSize+3, 0, imageSize, imageSize)];
       
        remarkView.layer.cornerRadius=3.0f;
        remarkView.layer.masksToBounds=YES;
        remarkView.layer.borderWidth=0.3f;
        remarkView.layer.borderColor=[[UIColor lightGrayColor]colorWithAlphaComponent:0.4f].CGColor;
        
        remarkView.image = [UIImage imageNamed:@"defuleImg"];
        ...
        for(NSDictionary *dic in _friendsDataSource[@"friends"]){
            ...
            if([dic[@"hxid"] isEqualToString:buddy.username]){
                ...
                [footerHead setImageWithURL:imgUrl placeholderImage:[UIImage imageNamed:@"placeholder-avatar"]];
                remarkView.remark = dic[@"nick"];
                remarkView.image=footerHead.image;
            }
